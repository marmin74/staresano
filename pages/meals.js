...
export default function Meals() {
  ...
  const [profiloEsistente, setProfiloEsistente] = useState(false)
  ...
  useEffect(() => {
    const fetchUserAndCheckProfile = async () => {
      ...
      if (!profilo) {
        // 👇 Redirect automatico se manca il profilo
        alert("Devi prima completare il tuo profilo utente.")
        router.push('/profilo')
        return
      }

      setProfiloEsistente(true)
      fetchMeals(user.id)
    }
  ...
  const addMeal = async () => {
    if (!user || !profiloEsistente) {
      setError('Devi prima completare il tuo profilo utente.')
      return
    }
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {user && <p style={{ color: 'green' }}>Utente: {user.email}</p>}
